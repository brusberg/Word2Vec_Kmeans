...
int * cpuGOLD(int n, int k, int iter, int dim, double * data, double * centroids){
  ...
  int b = 0;
  while(b < iter){
    //if(i % ((int)ceil((float)(iter/20)))==0)fprintf(stderr,"Iter%d:%f%% Done\n",i,((double)i/(double)iter)*100); 
    //for each vector
...
int main( int argc, char **argv ){
  ...
  GPUstart = clock();
  dim3 dimBlock(512);
  dim3 dimGrid(ceil((float)n/(float)512));
  ...
  cudaFree(centroidsD);
  cudaFree(dataD);
  // 2. Thread per Centroid ***************************************
  memcpy(centroidsGPU, centroidsSTD, k * dim * sizeof(double));
  GPUstart = clock();
  dimBlock.x = 512;
  dimGrid.x = ceil((float)n/(float)512);
